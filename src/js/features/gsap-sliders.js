/**
 * @file gsap-sliders.js
 * @summary Initializes draggable GSAP-powered sliders for elements marked with [data-gsap-slider-init].
 * @description For each slider, injects full aria attributes, computes slide width and snap points,
 * sets up Draggable with inertia and snapping, and wires prev/next controls. If the slider is not
 * needed (all slides fit in the viewport), all aria attributes and styles are cleaned up and the
 * slider is left in a static state. Active, in-view, and not-active states are kept in sync
 * on every drag, throw, and control click.
 */

/**
 * Finds all [data-gsap-slider-init] elements within the given scope, evaluates whether
 * each slider is needed, and sets up Draggable with snap points, aria state management,
 * and prev/next control handlers.
 *
 * @param {Document|HTMLElement} [scope=document] - The DOM element within which to search.
 */

export function initGsapSliders(scope = document) {
  const elements = scope.querySelectorAll("[data-gsap-slider-init]");
  if (!elements.length) return;

  elements.forEach((element) => {
    // Kill any existing Draggable instance before re-initializing
    if (element._sliderDraggable) element._sliderDraggable.kill();

    const collection = element.querySelector("[data-gsap-slider-collection]");
    const track = element.querySelector("[data-gsap-slider-list]");
    const items = element.querySelectorAll("[data-gsap-slider-item]");
    const controls = element.querySelectorAll("[data-gsap-slider-control]");

    /* ---------------------------------- */
    /* Inject aria attributes             */
    /* ---------------------------------- */

    element.setAttribute("role", "region");
    element.setAttribute("aria-roledescription", "carousel");
    element.setAttribute("aria-label", "Slider");

    collection.setAttribute("role", "group");
    collection.setAttribute("aria-roledescription", "Slides List");
    collection.setAttribute("aria-label", "Slides");

    items.forEach((slide, i) => {
      slide.setAttribute("role", "group");
      slide.setAttribute("aria-roledescription", "Slide");
      slide.setAttribute("aria-label", `Slide ${i + 1} of ${items.length}`);
      slide.setAttribute("aria-hidden", "true");
      slide.setAttribute("aria-selected", "false");
      slide.setAttribute("tabindex", "-1");
    });

    controls.forEach((btn) => {
      const dir = btn.getAttribute("data-gsap-slider-control");
      btn.setAttribute("role", "button");
      btn.setAttribute(
        "aria-label",
        dir === "prev" ? "Previous Slide" : "Next Slide"
      );
      btn.disabled = true;
      btn.setAttribute("aria-disabled", "true");
    });

    /* ---------------------------------- */
    /* Determine if slider is needed      */
    /* ---------------------------------- */

    const styles = getComputedStyle(element);
    const statusVar = styles.getPropertyValue("--slider-status").trim();
    let spvVar = parseFloat(styles.getPropertyValue("--slider-spv"));

    const rect = items[0].getBoundingClientRect();
    const marginRight = parseFloat(getComputedStyle(items[0]).marginRight);
    const slideW = rect.width + marginRight;

    if (isNaN(spvVar)) spvVar = collection.clientWidth / slideW;

    const spv = Math.max(1, Math.min(spvVar, items.length));
    const sliderEnabled = statusVar === "on" && spv < items.length;

    console.log(element, sliderEnabled);

    element.setAttribute(
      "data-gsap-slider-status",
      sliderEnabled ? "active" : "not-active"
    );

    if (!sliderEnabled) {
      // Teardown — strip all aria and inline styles when the slider is not needed
      track.removeAttribute("style");
      track.onmouseenter = null;
      track.onmouseleave = null;
      track.removeAttribute("data-gsap-slider-list-status");

      element.removeAttribute("role");
      element.removeAttribute("aria-roledescription");
      element.removeAttribute("aria-label");

      collection.removeAttribute("role");
      collection.removeAttribute("aria-roledescription");
      collection.removeAttribute("aria-label");

      items.forEach((slide) => {
        slide.removeAttribute("role");
        slide.removeAttribute("aria-roledescription");
        slide.removeAttribute("aria-label");
        slide.removeAttribute("aria-hidden");
        slide.removeAttribute("aria-selected");
        slide.removeAttribute("tabindex");
        slide.removeAttribute("data-gsap-slider-item-status");
      });

      controls.forEach((btn) => {
        btn.disabled = false;
        btn.removeAttribute("role");
        btn.removeAttribute("aria-label");
        btn.removeAttribute("aria-disabled");
        btn.removeAttribute("data-gsap-slider-control-status");
      });

      return;
    }

    /* ---------------------------------- */
    /* Track hover state                  */
    /* ---------------------------------- */

    track.onmouseenter = () =>
      track.setAttribute("data-gsap-slider-list-status", "grab");
    track.onmouseleave = () =>
      track.removeAttribute("data-gsap-slider-list-status");

    /* ---------------------------------- */
    /* Bounds and snap points             */
    /* ---------------------------------- */

    const vw = collection.clientWidth;
    const tw = track.scrollWidth;
    const maxScroll = Math.max(tw - vw, 0);
    const minX = -maxScroll;
    const maxX = 0;
    const maxIndex = maxScroll / slideW;
    const full = Math.floor(maxIndex);

    // Build snap points — one per full slide step, plus a final fractional step if needed
    const snapPoints = [];
    for (let i = 0; i <= full; i++) snapPoints.push(-i * slideW);
    if (full < maxIndex) snapPoints.push(-maxIndex * slideW);

    let activeIndex = 0;
    const setX = gsap.quickSetter(track, "x", "px");
    let collectionRect = collection.getBoundingClientRect();

    /* ---------------------------------- */
    /* Status updater                     */
    /* ---------------------------------- */

    function updateStatus(x) {
      if (x > maxX || x < minX) return;

      // Find the snap point closest to the current x position
      const calcX = Math.min(maxX, Math.max(minX, x));
      let closest = snapPoints[0];
      snapPoints.forEach((pt) => {
        if (Math.abs(pt - calcX) < Math.abs(closest - calcX)) closest = pt;
      });
      activeIndex = snapPoints.indexOf(closest);

      // Update slide visibility and selection state
      items.forEach((slide, i) => {
        const r = slide.getBoundingClientRect();
        const slideCenter = r.left - collectionRect.left + r.width / 2;
        const inView = slideCenter > 0 && slideCenter < collectionRect.width;
        const status =
          i === activeIndex ? "active" : inView ? "inview" : "not-active";

        slide.setAttribute("data-gsap-slider-item-status", status);
        slide.setAttribute(
          "aria-selected",
          i === activeIndex ? "true" : "false"
        );
        slide.setAttribute("aria-hidden", inView ? "false" : "true");
        slide.setAttribute("tabindex", i === activeIndex ? "0" : "-1");
      });

      // Enable or disable prev/next controls based on position
      controls.forEach((btn) => {
        const dir = btn.getAttribute("data-gsap-slider-control");
        const can =
          dir === "prev"
            ? activeIndex > 0
            : activeIndex < snapPoints.length - 1;

        btn.disabled = !can;
        btn.setAttribute("aria-disabled", can ? "false" : "true");
        btn.setAttribute(
          "data-gsap-slider-control-status",
          can ? "active" : "not-active"
        );
      });
    }

    /* ---------------------------------- */
    /* Controls                           */
    /* ---------------------------------- */

    controls.forEach((btn) => {
      const dir = btn.getAttribute("data-gsap-slider-control");
      btn.addEventListener("click", () => {
        if (btn.disabled) return;
        const target = activeIndex + (dir === "next" ? 1 : -1);
        gsap.to(track, {
          duration: 1,
          ease: "power3.out",
          x: snapPoints[target],
          onUpdate: () => updateStatus(gsap.getProperty(track, "x")),
        });
      });
    });

    /* ---------------------------------- */
    /* Draggable                          */
    /* ---------------------------------- */

    element._sliderDraggable = Draggable.create(track, {
      type: "x",
      inertia: true,
      bounds: { minX, maxX },
      throwResistance: 2000,
      dragResistance: 0.05,
      maxDuration: 0.6,
      minDuration: 0.2,
      edgeResistance: 0.75,
      snap: { x: snapPoints, duration: 0.4 },
      onPress() {
        track.setAttribute("data-gsap-slider-list-status", "grabbing");
        collectionRect = collection.getBoundingClientRect();
      },
      onDrag() {
        setX(this.x);
        updateStatus(this.x);
      },
      onThrowUpdate() {
        setX(this.x);
        updateStatus(this.x);
      },
      onThrowComplete() {
        setX(this.endX);
        updateStatus(this.endX);
        track.setAttribute("data-gsap-slider-list-status", "grab");
      },
      onRelease() {
        setX(this.x);
        updateStatus(this.x);
        track.setAttribute("data-gsap-slider-list-status", "grab");
      },
    })[0];

    /* ---------------------------------- */
    /* Initial state                      */
    /* ---------------------------------- */

    setX(0);
    updateStatus(0);
  });
}
